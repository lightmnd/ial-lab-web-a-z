...
Questo documento contiene le versioni aggiornate e hardenate dei file `index.html` e `app.js` dell'app client-side. Prendi spunto da questo file per applicare le patch.
...
* Una Content Security Policy di base è stata aggiunta nel file `index.html`. Se la app verrà servita da un server, è preferibile impostare la CSP come header HTTP lato server.
